...
%4.0,table-figures-uncertainty = 1]S[table-format =
%4.0,table-figures-uncertainty = 1]}
\toprule
...
Der Wert für die Bandlücke $E_g$ ergibt sich schließlich aus allen vier $m$-Werten durch
...
\subsection{Bestimmung des Beweglichkeitsverhältnis}
Das Verhältnis der Beweglichkeiten $b$ der beiden Ladungsträger ist definiert als
\begin{equation}
 b = \frac{μ_n}{μ_p}
\end{equation}

Setzt man nun dieses in die \eref{hallkonst_lang} ein, so erhält man eine Funktion,
...
der Störstellenerschöpfung ($n = 0$) mit \eref{hallkonst_b}:
\begin{equation}
...
\end{equation}
Setzen wir dieses Ergebnis nun in \eref{hallkonst_b} ein, erhalten wir $R_{H,\mbox{ext.}}$.
\begin{equation}
...
Aus \fref{hallkonstante2} lassen sich folgende Werte ablesen:
\begin{eqnarray}
