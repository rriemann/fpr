...
\toprule
& {$SC_2$: $PM_2\wedge PM_4$} & $SC_3$: $PM_3\wedge PM_5$\\
\midrule
Rate & 151 & 204\\
Fehler & 12 & 14\\
\bottomrule
...
eine gemessene Spannung von \SI{2}{\volt}, was gut zu unserem Ergebnis in \fref{tac}
passt.

\begin{figure}[ht!]
  \centering
  \includegraphics[width=\columnwidth,keepaspectratio=true]{tac.png}
  \caption{Puls variabler Länger, der mittels des Zeitgeber-Moduls generiert wurde}
  \label{fig:tac}
\end{figure}


Das mit der Schaltung in \fref{zeit_schaltung} aufgenommene Histogramm ist in \fref{eichung} dargestellt.
...
Schaltung in \fref{muon_schaltung} verwendet, allerdings ohne das
...
\fref{ohne_verzoegerung} betrachtet werden.
...
Man kann erkennen, dass beinahe alle Events in den ersten \SI{4}{\micro\second}
...
Aus \fref{messung} kann die Rate der registrierten Myonen-Zerfälle abgeleitet.
...
Untergrundereignis zu detektieren, da laut \tref{SC2_SC3_koinzidenz} pro
...
die ungfähr zu erwartenden Werte festgelegt, die in \tref{fit_werte} gelistet
...
\bottomrule
\end{tabular*}
\caption{Startwerte und Ergebnisse des Fits von \eref{fit} in \fref{fit}}
\label{tab:fit_werte}
\end{table}
...
\begin{figure}[ht!]
  ...
  \caption{Langzeitmessung der μ-Zerfälle mit Hilfe der in \fref{muon_schaltung} dargestellten Schaltung}
  \label{fig:messung}
\end{figure}
...
Der Parameter χ/DoF liegt bei $1.20$ und damit sehr nahe bei 1. Der Fit liefert
...
vernachlässigt werden. Wie in \tref{systematik} zu sehen ist, führt ein kleinerer
...
zu dem Fehler aus \eref{ergebnis}, so erhält man:
\begin{equation}
 τ_μ = \uuline{(2,2 ± 0,2\,^{+0,0}_{-0,1}) \si{\micro\second}}
 \label{eqn:ergebnis_sys}
\end{equation}
% Lebensdauer mit \SI{0,2}{\micro\second} relativ groß ausfällt (auf jeden Fall
% größer als die oben abgeschätzten \SI{1}{\percent}) spielt die
...
Unser Ergebnis deckt sich also gut mit dem in \eref{tau_mu_theo} angegebenen
...
Gehen wir davon aus, dass die Formel in \eref{fit} näherungsweise ein einfaches,
