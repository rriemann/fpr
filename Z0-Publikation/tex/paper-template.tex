...
\usepackage[textsize=tiny,colorinlistoftodos%,disable
]{todonotes}
\begin{document}
\hugehead
...
factor to $N_C = \num{3.2(13)}$.
All resuls are in good agreement to the standard model and can be considered as
...
decays to two very unstable \todo{plural von τ?}\ tauons, these tauons decay very fast. Then two
...
By fitting the data
points the overall hadronic cross section $σ_0^{\mathrm{had}}$\todo{$σ_0^{had}$ haben wir doch schon. muss die 0 irgendwo weg?}, the mass $m_Z$ and the decay width $Γ_Z$
...
\end{tabular}
\vspace*{0.3cm}
\caption{\baselineskip=0.38cm results of fit on data. The hadronic cross section $σ_0^{\mathrm{had}}$, the mass $m_Z$ and the decay width $Γ_Z$ of the $Z^0$-boson is determined from the fit in \fref{fit_hadrons}.}
\label{tab:results}
\end{center}
\vspace*{-0.5cm}
\end{table}
...
width in \eref{leptonicwidth}. $Γ_{ν_l}$ can be determinde by using the
following relation:
\begin{eqnarray}
...
\begin{equation}
 ...
 \label{eqn:gammahad2}
\end{equation}
Replacing $Γ_{\mathrm{had}}$ in \eref{gammahad2} using \eref{gammahad1} leads to a
...
lower than around \SI{90}{\GeV}\todo{richtig?}.

\todo[inline,size=]{
kleine erläuterung zur abweichung, kleines schlusswort
